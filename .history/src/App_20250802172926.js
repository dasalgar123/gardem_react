...
import './App.css';
import { obtenerProductos, obtenerProductosConVariantes } from './services/api';
...
function App() {
  ...
  // Cargar productos por categoría con colores y tallas específicos
  useEffect(() => {
    const cargarProductosPorCategoria = async () => {
      if (filtroCategoria === 'Todas los Productos') {
        ...
        try {
          setLoading(true);
          const data = await obtenerProductosConVariantes(filtroCategoria);
          setProductos(data);
          setError(null);
        } catch (err) {
